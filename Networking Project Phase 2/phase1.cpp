...
int main(int argc, char* argv[])
{
	...
	for (int numExp = 0; numExp < 22; numExp++)
	{
		...
		double NAT = sentData::generateTimeToNext();	//Next Arrival Time
		double NDT = 0;				//Next Departure Time
		double downTime = 0.0;
		double totalSize = 0;
		int dropped = 0;

		queue<ACK> buffer;	//NOTE: top of buffer is link node
		list<packet> GEL;
		...
		while (currentTime < 100000)
		{
			...
			if ((NAT < NDT) || buffer.empty())
			{
				...
				//Adds new sentData with time NAT to GEL
				GEL.push_back(sentData(currentTime));
				...
				//Generate time for the sentData of next packet
				NAT = currentTime + sentData::generateTimeToNext();
			}
			else 		//Departure is the next event to occur
			{
				...
				//Add ACK event to list of packet
				GEL.push_back(buffer.front());
				//Set the time of the the event to current time
				GEL.back().setTimeOE(currentTime);
				//remove ACK event from link node
				buffer.pop();

				//Set NDT to the time of the NEXT ACK (currentTime + serviceTime)
				if (!buffer.empty())
					NDT = currentTime + buffer.front().getServiceTime();
				//cout << "d";
			}
